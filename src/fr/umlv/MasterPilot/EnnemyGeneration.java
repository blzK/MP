    ...
    public ArrayList<SpaceShuttle> Generate(String vague) {
        ...
        while (itkey.hasNext()) {
            ...
            if(key.contains("Wave"+vague)){
                if(key.contains("Tie")){
                    for(i=0;i<val;i++){ this.listEnnemy.add(new EnnemyShuttle1(-200, 0, world)); }
                }
                else if(key.contains("Cruiser")){
                    for(i=0;i<val;i++){ this.listEnnemy.add(new EnnemyShuttle2(-400, 0, world)); }
                }
                else if(key.contains("Squadron")){
                    for(i=0;i<val;i++){ this.listEnnemy.add(new EnnemyShuttle3(0, 200, world)); }
                }
                else if(key.contains("FakePlanet")){
                    for(i=0;i<val;i++){ this.listEnnemy.add(new EnnemyShuttle4(0, 0, world)); }
                }
                else{ // 5 type
                    for(i=0;i<val;i++){ this.listEnnemy.add(new EnnemyShuttle5(0, 0, world)); }
                }
            }
            isGenerated=true;
        }
